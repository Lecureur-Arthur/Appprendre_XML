...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="pays">
        ...
        <xsl:choose>
            <xsl:when test="@nomPays = 'USA'">
                <td id="USA"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'GrandeBretagne'">
                <td id="GrandeBretagne"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'Chine'">
                <td id="Chine"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'Russie'">
                <td id="Russie"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'Allemagne'">
                <td id="Allemagne"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'Japon'">
                <td id="Japon"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'France'">
                <td id="France"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'CoreeDuSud'">
                <td id="CoreeDuSud"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'Italie'">
                <td id="Italie"><xsl:value-of select="." /></td>
            </xsl:when>
            <xsl:when test="@nomPays = 'Australie'">
                <td id="Australie"><xsl:value-of select="." /></td>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="totalMedaille">
        <td><xsl:value-of select='sum(../medaille)'/></td>
